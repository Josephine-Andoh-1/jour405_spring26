...
The correlation matrix shows that a handful of the variables are correlated. The percentage of people with a bachelor's degree or higher and income are relatively positively correlated. On the other hand, the percentage of white people and population are relatively negatively correlated. Unemployment and voter turnout are also relatively negatively correlated. The percentage of white people in a county and income are almost completely uncorrelated, and so are the percentage of people with a bachelor's degree or higher and median age.

The scatterplots show that all the independent variables seem to have a linear relationship with the dependent variable, but some relationships are stronger than others. Income, median age, unemployment and the percentage of white people in a county seem to have a relatively strong relationship with voter turnout. On the other hand, population and the percentage of people with a bachelor's degree or higher seem to have a relatively weaker relationship with voter turnout.
...
This multiple variable model predicts voter turnout with reasonable accuracy, according to its output. The model overall is statistically significant, and so is the relationship between income, median age or the percentage of white people in a county and voter turnout. However, according to the model, the relationship between population, higher education levels or unemployment and voter turnout is not statistically significant.

The range of the model's residuals is very small, and its median is almost zero. It has a very low residual standard error and high multiple and adjusted R-squared values. The model also has a high F-statistic. Based on what I've seen, this model is useful because most of its values demonstrate accuracy.
...
# p-values are on the far right of the model
```
...
When I adjusted the model to incorporate only income, unemployment, median age and racial composition, it had an adjusted R-squared value of approximately 0.82, the highest I found. Though that is not much higher than the initial model, it is still an increase in accuracy.

When I adjusted the model to incorporate only population, unemployment, median age and racial composition, it had an adjusted R-squared value of approximately 0.63, the lowest I found. When I adjusted the model to incorporate only income, median age and racial composition, it had an adjusted R-squared value of approximately 0.8024, only slightly more accurate than the initial model.
...
I could use this analysis method to cover an election or voter participation story in different ways. I could plug a Maryland county's demographic information into this analysis method to predict voter turnout in that county. I could also update this analysis method based on new demographic information from a given Maryland county (updated Census information, for example) to ensure its continued accuracy.
