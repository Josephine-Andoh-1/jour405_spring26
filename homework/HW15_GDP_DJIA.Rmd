...
The U.S. economy in 2013 had begun to emerge from the Great Depression and heated up in the years before the COVID-19 pandemic hit. The government and private sector shut down in a bid to save lives, ease overcrowded hospitals and prevent a wider spread of a little-known but often debilitating and deadly virus. As economic life returned to pre-pandemic levels, the outlook for the economy mostly was positive, but inflation remained high for several years.
...
```{r}
econ <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/gdp_djia.csv")
```
...
```{r}
econ |> summarize(correlation = cor(GDP, DJIA, method="pearson"))
```
...
econ |> 
  ...
  labs(
    title = "GDP and DJIA strongly correlated from 2013 to 2024, according to economic data")
```
...
There was a strong correlation between the Gross Domestic Project and the Dow Jones Industrial Average between 2013 and 2024. This means that an increase or decrease in the GDP could almost always predict an increase or decrease in the DJIA during that time period. The scatterplot shows data points that all cluster around a diagonal line (more or less) with a rather small range of uncertainty.
