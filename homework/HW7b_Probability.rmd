...
wh_paid |>
  ggplot() +
  geom_histogram(aes(x = SALARY), binwidth = 35000) +
  geom_vline(aes(xintercept = mean(SALARY)), color = "red", linetype = "dashed", linewidth = 1) +
  geom_vline(aes(xintercept = mean(SALARY) - sd(SALARY)), color = "blue", linetype = "dashed", linewidth = 1) + 
  geom_vline(aes(xintercept = mean(SALARY) + sd(SALARY)), color = "blue", linetype = "dashed", linewidth = 1)
```

**Describe the shape of the distribution. Is it roughly normal? Skewed? Where do most salaries cluster?**

The dataset appears to be positively skewed. Most salaries are clustered between $74,500 and $101,500.  
...
The data follows the empirical rule relatively closely, meaning that the dataset has an almost-normal distribution. The standard deviations show that there are no extremely unusual values in the dataset since none of them have a z-score higher than three.
...
There is an 18% chance that a randomly selected 2025 White House employee makes over $150,000 per year. The normal approximation is slightly lower than the actual percentage.
...
According to the normal approximation, approximately 36.6% of 2025 White House employees should make between $80,000 and $120,000 per year. However, only around 32.6% do. This "middle range" tells us, once again, that the White House salary structure has an almost-normal distribution. However, it has more values outside the "middle range" than a normally-distributed dataset would.
...
Reality differs most from what the normal distribution predicts concerning salaries between $60,000 to $90,000 per year. This might be explained by the large number of managers and associate directors who worked at the White House in 2025, who mostly made $74,500 and $83,500 per year, respectively.

Another category where reality differs from prediction is salaries below $60,000. This might be explained by the fact that White House employees and detailees are most likely college-educated, meaning they have to earn (somewhat) competitive salaries.
...
2025 White House employees have 16% chance of making over $150,000 per year (headline) 

White House employees had a 16% chance of making over $150,000 per year in 2025, according to probability analysis. (lede)


To advance this story, I could compare the probability of a White House employee making over $150,000 annually over many years, specifically focusing on 2024. I could also analyze the actual percentages of White House employees who make over $150,000 annually over the years to see if or how it has changed. Additionally, I could analyze which positions make employees more or less likely to make over $150,000 per year.
